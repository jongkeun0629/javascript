function createAccount(initialBalance = 0) {
  return {
    balance: initialBalance,
  };
}

function deposit(account, amount) {
  if (amount <= 0) throw new Error("입금 금액은 0보다 커야 합니다.");
  return { ...account, balance: account.balance + amount };
}

function withdraw(account, amount) {
  if (amount <= 0) throw new Error("출금 금액은 0보다 커야 합니다.");
  if (amount > account.balance) throw new Error("잔액이 부족합니다.");
  return { ...account, balance: account.balance - amount };
}

let account = createAccount(1000);
console.log(`초기 잔액: ${account.balance}`);
account = deposit(account, 500);
console.log(`입금 후 잔액: ${account.balance}`);
account = withdraw(account, 300);
console.log(`출금 후 잔액: ${account.balance}`);
try {
  account = withdraw(account, 1500);
} catch (error) {
  console.error(error.message);
}
console.log(`최종 잔액: ${account.balance}`);
